...
nodes <- read.csv("./fastText/edu-out/nodes.csv", header=T, as.is=F)

links <- read.csv("./fastText/edu-out/links.csv", header=T, as.is=F)
...
#                    labels="",
#                    edge_color = rgb(0,0,0,alpha = (E(net)$sim)^(5/2)))
...
closeCentrality <- closeness(net, vids = V(net), mode = "all", normalized = FALSE)

sort(closeCentrality, decreasing = TRUE)[50:100]

strengthDegree <- strength(net)

sort(strengthDegree, decreasing = FALSE)[0:100]

sort(strengthDegree, decreasing = TRUE)[0:100]

library(wordVectors)

model = train_word2vec("book.txt",
                           output="corpus.bin", threads = 4,
                            vectors = 100, window=20, cbow=1, min_count = 10, force= TRUE)

# model_cds = read.vectors("corpus.bin")

cosineSimilarity(model_cds)

dog <- model[rownames(model)=="dog",]
cat <- model[rownames(model)=="door",]
