...
interaction_list <- read.delim("intersection.txt", header = FALSE, sep = "\t", dec = ".")
names(interaction_list) <- c('word')
interaction_list
rebecca_list <- read.delim("rebecca_words.txt", header = FALSE, sep = "\t", dec = ".")
names(rebecca_list) <- c('word')
rebecca_v <- rebecca_list %>%
  filter(word %in% interaction_list$word)

# write strenth rank to file
strength_vec = c()
...
write(strength_vec, file = "strength_rank")

# write rebecca rank to file

print(as.character(strength_vec))

print(typeof(rebecca_vec))
rebecca_vec = c()
for (word in as.character(rebecca_v$word)) {
  rebecca_vec <- c(rebecca_vec, word)
}

write(rebecca_vec, file = "rebecca_rank", ncolumns = )
